 ...
 */
 ...
 /** Compile: kotlinc 11.kt -include-runtime -d 11.jar 
   * Run:     java -jar 11.jar 
   */
fun main(args: Array<String>): Unit {
    ...
    val zeroRow: IntArray = IntArray(nums[0].size + 3) { 0 }
    val padded: Array<IntArray> = Array(nums.size + 3) { i -> if (i < 3) zeroRow else IntArray(nums[i - 3].size + 3) { j -> if (j < 3) 0 else nums[i - 3][j - 3] } }

    var max: Int = 0;

    for (row in 3..padded.lastIndex) {
        for (col in 3..padded[row].lastIndex) {
            var up = 1
            var left = 1
            var diagBack = 1
            var diagFwd = 1
            for (delta in 0..3) {
                up *= padded[row - delta][col]
                left *= padded[row][col - delta]
                diagBack *= padded[row - delta][col - delta]
                diagFwd *= padded[row - (3 - delta)][col - delta]
            }

            max = maxOf(max, up, left, diagBack, diagFwd)
        }
    }
